            ...
            {
                {
                    ...
                    // Description 为 WCHAR[128]（宽字符），窄字符 logger 不能直接格式化宽字符，先转成 UTF-8 窄字符串
                    char adapterName[256] = {};
                    WideCharToMultiByte(CP_UTF8, 0, dxgiAdapterDesc.Description, -1, adapterName, sizeof(adapterName), nullptr, nullptr);

                    MIKU_CORE_INFO("Found D3D12 Adapter {}: {}", adpeterIndex, adapterName);
    ...
    bool D3D12Device::Init(D3D12RenderBackend* backend, D3D12Adapter* adapter) 
    {
       ...
       // ResourceDescriptorHeap/SamplerDescriptorHeap must be supported on devices that support both D3D12_RESOURCE_BINDING_TIER_3 and D3D_SHADER_MODEL_6_6
            ...
            // Sampler descriptor heap
        {
                ...
                for (int i = 0; i < D3D12_RENDER_BACKEND_BINDLESS_MAX_NUM_SAMPLER_DESCRIPTORS; i++)
                {
                    freeSamplerDescriptorIndices.push_back(D3D12_RENDER_BACKEND_BINDLESS_MAX_NUM_SAMPLER_DESCRIPTORS - i - 1);
                }
        }
